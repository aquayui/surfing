    ...
    for _ in temp:
        ...
        if judge_Internet(job_type):
            ...
            sql = "INSERT INTO recruitment_1 (provider,location,salary,experience,education,company_type,job_type,description,job_name,company,number,date,job_class) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s');" % (
                '前程无忧', location, salary, experience, education, company_type, job_type, description, job_name,
                company_name, number, date, job_class)
            # 执行SQL语句
            try:
                cursor.execute(sql)
                conn.commit()
...
def get_detail_url(queue):
    for page in range(1,2001):
        ...
        # queue.put("http://testedu.com/{id}".format(id=page))#Queue队列的put方法用于向Queue队列中放置元素，由于Queue是先进先出队列，所以先被Put的URL也就会被先get出来。
...
if __name__ == "__main__":
    detail_url_queue = Queue(maxsize=10000) #用Queue构造一个大小为1000的线程安全的先进先出队列
