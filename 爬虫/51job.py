...
def parse_data(response):
    ...
    #         sql = "INSERT INTO `recruitment` (`title`, `location`,`companytype`) VALUES (%s,%s,%s,%s, %s)"
    #         cursor.execute(sql, (zp_info_title, zp_info_location, zp_info_type)
    #         conn.commit()
    # finally:
    #     conn.close()

def main(num):
    url=f'https://search.51job.com/list/010000,000000,0000,00,9,99,+,2,{num}.html?lang=c&postchannel=0000&workyear=99&cotype=99&degreefrom=99&jobterm=99&companysize=99&ord_field=0&dibiaoid=0&line=&welfare='
    html=get_data(url)
    if html:
